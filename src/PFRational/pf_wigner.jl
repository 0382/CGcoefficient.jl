...
function _pf_6j(j1::Int, j2::Int, j3::Int, j4::Int, j5::Int, j6::Int)
    check_6j(2j1, 2j2, 2j3, 2j4, 2j5, 2j6) || return zero(SqrtRational{BigInt})
    A = one(PFRational{Int16})
    sn = one(BigInt)
    _pf_6j_impl!(A, sn, j1, j2, j3, j4, j5, j6)
    iszero(sn) && return zero(SqrtRational{BigInt})
    sd = one(BigInt)
    rn = one(BigInt)
    rd = one(BigInt)
...
function _pf_f6j(j1::Int, j2::Int, j3::Int, j4::Int, j5::Int, j6::Int)::Float64
    check_6j(2j1, 2j2, 2j3, 2j4, 2j5, 2j6) || return zero(Float64)
    A = one(PFRational{Int16})
    sn = one(BigInt)
    _pf_6j_impl!(A, sn, j1, j2, j3, j4, j5, j6)
    iszero(sn) && return zero(Float64)
