...
# 返回 (-1)^x
iphase(x::T) where {T<:Integer} = iseven(x) ? one(T) : -one(T)

# 判断是否同为奇数或者同为偶数
is_same_type(x::T, y::T) where {T<:Integer} = iseven(x ⊻ y)

# 将整数除以 2，并显示结果
show_half(x::Integer) = iseven(x) ? "$(x>>1)" : "$x/2"

# 检查 j,m 是否匹配
check_jm(dj::T, dm::T) where {T<:Integer} = is_same_type(dj, dm) & (abs(dm) <= dj)
# j,m 不匹配报错信息
miss_match_msg(dj::T, dm::T) where {T<:Integer} = begin
    "j = $(show_half(dj)), m = $(show_half(dm)) are not matched" 
end

# 检查 j1, j2 是否能够耦合到 j3
check_couple(dj1::T, dj2::T, dj3::T) where {T<:Integer} = begin
    (dj1 >= 0) & (dj2 >= 0) & is_same_type(dj1+dj2, dj3) & (abs(dj1-dj2) <= dj3 <= dj1+dj2)
end
# j1,j2 不能耦合到 j3 时的报错信息
miss_couple_msg(dj1::T, dj2::T, dj3::T) where {T<:Integer} = begin
    "$(show_half(dj1)), $(show_half(dj2)) cannnot couple to $(show_half(dj3))"
end


# 计算 CG 系数，参数为量子数的两倍（为了保证是整数）
function _dCG(dj1::BigInt, dj2::BigInt, dj3::BigInt, dm1::BigInt, dm2::BigInt, dm3::BigInt)
    check_jm(dj1, dm1) || error(miss_match_msg(dj1, dm1))
    check_jm(dj2, dm2) || error(miss_match_msg(dj2, dm2))
    check_jm(dj3, dm3) || error(miss_match_msg(dj3, dm3))
...
CG(j1::AngularType, j2::AngularType, j3::AngularType,
   m1::AngularType, m2::AngularType, m3::AngularType) = _dCG(BigInt.((2j1,2j2,2j3,2m1,2m2,2m3))...)
threeJ(j1::AngularType, j2::AngularType, j3::AngularType,
       m1::AngularType, m2::AngularType, m3::AngularType) = _d3j(BigInt.((2j1,2j2,2j3,2m1,2m2,2m3))...)
sixJ(j1::AngularType, j2::AngularType, j3::AngularType,
     j4::AngularType, j5::AngularType, j6::AngularType) = _d6j(BigInt.((2j1,2j2,2j3,2j4,2j5,2j6))...)
nineJ(j1::AngularType, j2::AngularType, j3::AngularType,
      j4::AngularType, j5::AngularType, j6::AngularType,
      j7::AngularType, j8::AngularType, j9::AngularType) = _d9j(BigInt.((2j1,2j2,2j3,2j4,2j5,2j6,2j7,2j8,2j9))...)
